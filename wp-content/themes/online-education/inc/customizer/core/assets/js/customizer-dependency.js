...
(
	function ( $ ) {
		 ...
		 */
		var Online_Education_Customizer_FrameWork = {
			...
			checkControlVisibility : function ( control, id ) {
				...
				if ( ! _.isUndefined( control ) ) {
					...
					if ( 'undefined' != typeof OnlineEducationCustomizerControlsToggle[id] ) {
						var check            = false,
						    dependency_param = OnlineEducationCustomizerControlsToggle[id],
						...
						if ( 'undefined' !== typeof conditions ) {
							...
							if ( ! check ) {
								control.container.addClass( 'online-education-hide' );
							} else {
								control.container.removeClass( 'online-education-hide' );
							}
						}
					}
			...
			checkDependency : function ( conditions, values, compare_operator ) {
				...
				if ( _.isString( testValue ) ) {
					...
					if ( ! _.isUndefined( OnlineEducationCustomizerControlsToggle[testValue] ) ) {
						var conditions = ! _.isUndefined( OnlineEducationCustomizerControlsToggle[testValue]['conditions'] ) ? OnlineEducationCustomizerControlsToggle[testValue]['conditions'] : OnlineEducationCustomizerControlsToggle[testValue];
						var operator   = ! _.isUndefined( OnlineEducationCustomizerControlsToggle[testValue]['operator'] ) ? OnlineEducationCustomizerControlsToggle[testValue]['operator'] : 'AND';
						...
						if ( ! _.isUndefined( conditions ) ) {
							...
							if ( ! control.checkDependency( conditions, values, operator ) ) {
								...
								control_obj.container.removeClass( 'online-education-hide' );
							}
						}
					}
					...
					$.each(
						...
						function ( index, val ) {
							...
							if ( 'undefined' !== typeof OnlineEducationCustomizerControlsToggle[cond_key] ) {
								var conditions = ! _.isUndefined( OnlineEducationCustomizerControlsToggle[cond_key]['conditions'] ) ? OnlineEducationCustomizerControlsToggle[cond_key]['conditions'] : OnlineEducationCustomizerControlsToggle[cond_key];
								var operator   = ! _.isUndefined( OnlineEducationCustomizerControlsToggle[cond_key]['operator'] ) ? OnlineEducationCustomizerControlsToggle[cond_key]['operator'] : 'AND';
			...
			hideEmptySections : function () {
				...
				$( 'ul.accordion-section.control-section-online-education_section' ).each(
					function () {
						...
						if ( controls.length > 0 ) {
							controls.each(
								function () {
									if ( ! $( this ).hasClass( 'online-education-hide' ) && $( this ).css( 'display' ) != 'none' ) {
										visibleIt = true;
									}
								}
							);
							...
							if ( ! visibleIt ) {
								$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'online-education-hide' );
							} else {
								$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'online-education-hide' );
							}
						}
					}
				);
		...
		$(
			function () {
				Online_Education_Customizer_FrameWork.init();
			}
		);
