 ...
 * Helper class to enqueue fonts.
 *
 * Class Online_Education_Generate_Fonts
 */
class Online_Education_Generate_Fonts {
	...
	public static function get_fonts() {
		...
		do_action( 'online_education_get_fonts' );

		return apply_filters( 'online_education_add_fonts', self::$fonts );
	...
	public static function render_fonts() {
		...
		$font_list = apply_filters( 'online_education_render_fonts', self::get_fonts() );

		$google_fonts = array();
		$font_subset  = array();

		$system_fonts = Online_Education_Fonts::get_system_fonts();
		...
		foreach ( $font_list as $name => $font ) {
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				...
				// Add Subset.
				$subset = apply_filters( 'online_education_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
		...
		wp_enqueue_style( 'online_education_googlefonts', $google_font_url, array(), ONLINE_EDUCATION_VERSION, 'all' );
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		...
		$fonts = apply_filters( 'online_education_google_fonts_selected', $fonts );
