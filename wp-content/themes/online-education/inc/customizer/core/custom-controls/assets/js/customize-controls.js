...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['online-education-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initOnline_EducationBackgroundControl();

			},

			initOnline_EducationBackgroundControl : function () {

				var control     = this,
				    value       = control.setting._value,
				    colorpicker = control.container.find( '.online-education-color-picker-alpha' );
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( OnlineEducationCustomizerControlBackground.placeholder );
					}
 ...
 * @package Online_Education
 */
wp.customize.controlConstructor[ 'online-education-buttonset' ] = wp.customize.Control.extend( {
 ...
 * @package Online_Education
 */
wp.customize.controlConstructor['online-education-dimensions'] = wp.customize.Control.extend( {
 ...
 * @package Online_Education
 */
wp.customize.controlConstructor[ 'online-education-editor' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['online-education-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					'use strict';
					...
					control.initOnline_EducationFontawesomeControl();
				},

				initOnline_EducationFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['OnlineEducationCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['online-education-group'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				this.container.on( 'online_education_settings_changed', control.onOptionChange );
			...
			registerToggleEvents : function () {
				...
				$( '.wp-full-overlay-sidebar-content' ).click(
					function ( e ) {
						if ( ! $( e.target ).closest( '.online-education-field-settings-modal' ).length ) {
							$( '.online-education-group-toggle-icon.open' ).trigger( 'click' );
						}
					}
				);
				...
				control.container.on(
					'click',
					'.online-education-group-wrap .online-education-group-toggle-icon',
					function ( e ) {
						...
						var $this          = $( this ),
						    parent_wrap    = $this.closest( '.customize-control-online-education-group' ),
						    is_loaded      = parent_wrap.find( '.online-education-field-settings-modal' ).data( 'loaded' ),
						    parent_section = parent_wrap.parents( '.control-section' );

						if ( $this.hasClass( 'open' ) ) {
							parent_wrap.find( '.online-education-field-settings-modal' ).hide();
						} else {

							/* Close popup when another popup is clicked */
							var get_open_popup = parent_section.find( '.online-education-group-toggle-icon.open' );
							if ( get_open_popup.length > 0 ) {
								get_open_popup.trigger( 'click' );
							}

							if ( is_loaded ) {
								parent_wrap.find( '.online-education-field-settings-modal' ).show();
							} else {

								var fields     = control.params.online_education_fields,
								    modal_wrap = $( OnlineEducationCustomizerControlGroup.group_modal_tmpl ),
								    device     = $( '#customize-footer-actions .active' ).attr( 'data-device' );

								parent_wrap.find( '.online-education-field-settings-wrap' ).append( modal_wrap );
								parent_wrap.find( '.online-education-fields-wrap' ).attr( 'data-control', control.params.name );
								control.online_education_render_field( parent_wrap, fields, control );
								parent_wrap.find( '.online-education-field-settings-modal' ).show();
				...
				control.container.on(
					'click',
					'.online-education-group-wrap > .customizer-text',
					function ( e ) {

						e.preventDefault();
						e.stopPropagation();

						$( this ).find( '.online-education-group-toggle-icon' ).trigger( 'click' );
			...
			online_education_render_field : function ( wrap, fields, control_element ) {

				var control             = this;
				var online_education_field_wrap = wrap.find( '.online-education-fields-wrap' );
				var fields_html         = '';
				var control_types       = [];
				...
				if ( 'undefined' != typeof fields.tabs ) {
					...
					var counter = 0;
					fields_html += '<div id="' + control_element.params.name + '-tabs" class="online-education-group-tabs">';

					fields_html += '<ul class="online-education-group-list">';
					_.each(
						fields.tabs,
						function ( value, key ) {
							var li_class = '';
					...
					fields_html += '<div class="online-education-tab-content" >';
					_.each(
						fields.tabs,
					...
					online_education_field_wrap.html( fields_html );
				...
				_.each(
					...
					function ( control_type, index ) {
						...
						switch ( control_type.key ) {
							...
							case 'online-education-color':
								control.initColorControl( online_education_field_wrap, control_element, control_type.name );
								break;

							case 'online-education-background':
								control.initBackgroundControl( control_element, control_type, control_type.name );
								break;

							case 'online-education-typography':
				...
				wrap.find( '.online-education-field-settings-modal' ).data( 'loaded', true );
			...
			generateFieldHtml : function ( fields_data, field_values ) {
				...
				_.each(
					...
					function ( attr, index ) {
						...
						if ( (
							     'online-education-typography' === attr.control
						     ) && controlsType.includes( attr.control ) ) {
							attr.languages = OnlineEducationCustomizerControlTypographySubsets;
						}
			...
			initColorControl : function ( wrap, control_elem, name ) {
				...
				var control     = this;
				var colorpicker = wrap.find( '.customize-control-online-education-color .online-education-color-picker-alpha' );
				...
				colorpicker.wpColorPicker(
					{
						change : function ( event, ui ) {
							...
							if ( 'undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha ) {
								...
								var picker  = $( '#' + name + '.customize-control-online-education-color .online-education-color-picker-alpha' );
								...
								control.container.trigger(
									'online_education_settings_changed',
									[
										control,
										$( element ),
										current,
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( OnlineEducationCustomizerControlTypography ) ) {
					return OnlineEducationCustomizerControlTypography;
				}
 ...
 * @package Online_Education
 */
wp.customize.controlConstructor['online-education-sortable'] = wp.customize.Control.extend( {
 ...
 * @package Online_Education
 */
wp.customize.controlConstructor['online-education-typography'] = wp.customize.Control.extend( {
